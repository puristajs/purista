...
- keep `src/harness` as the root for portable Harness definitions
- keep schemas beside the Framework or Harness boundary that owns them
...
- `purista/examples/agent-example` is the canonical lightweight example for
  native Harness service mounting. It must keep the definition portable, use
  `FakeModelProvider` for standalone tests, use address-first PURISTA
  consumers, and bind concrete providers only in the composition root.

## Review Cues
- CLI generated tests compile against current APIs.
- Generated Harness definitions use `@purista/harness/testing`; Framework wrappers use normal PURISTA context mocks.
