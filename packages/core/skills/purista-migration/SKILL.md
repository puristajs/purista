...
- When a PURISTA major is declared as a clean break, migrate the application
  to the new public API directly. Do not add framework compatibility aliases,
  deprecated forwarding methods, dual execution paths, legacy protocol
...
2. **Plan the target.** Pin the intended package set and write a
   migration ledger: change, reason, affected boundary, verification, rollback
   point, and owner. For a clean breaking major, list each removed API and its
   direct replacement without proposing a compatibility layer. Split
   application-code, infrastructure, and consumer changes; do not deploy them
   as one unexplained edit.
