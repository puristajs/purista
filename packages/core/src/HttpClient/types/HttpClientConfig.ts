...
import { Logger, LogLevelName } from '../../core'

/**
 * Tha basic configuration for a HTTPClient
 * Requires as least a `baseUrl`
 */
export type HttpClientConfig<CustomConfig extends Record<string, unknown>> = {
  /**
  ...
  name?: string
  /**
   * If you like to extend the HTTPClient class, you can add your own config here
   * @example ```typescript
   * // your config
   * type MyCustomConfig = {
   *    property: string
   ...
   *
   * // extended class
   * class MyClient extends HttpClient<MyCustomConfig> {
   *
   *  method(){
