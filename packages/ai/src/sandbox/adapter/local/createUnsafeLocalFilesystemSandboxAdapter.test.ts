...
import { createUnsafeLocalFilesystemSandboxAdapter } from './createUnsafeLocalFilesystemSandboxAdapter.js'

describe('createUnsafeLocalFilesystemSandboxAdapter', () => {
	it('reads and writes files only within root', async () => {
		const root = await mkdtemp(join(tmpdir(), 'purista-sandbox-local-'))
		const adapter = createUnsafeLocalFilesystemSandboxAdapter(root)
