...
export const aiWorkloadQueuePayloadSchema = extendApi(
	z.object({
		...
		tenantId: extendApi(z.string().optional(), { title: 'Optional tenant scope for memory and runtime isolation' }),
		principalId: extendApi(z.string().optional(), {
			title: 'Optional principal scope for memory and runtime isolation',
		}),
