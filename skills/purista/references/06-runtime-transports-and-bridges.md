...
Generated projects provide `src/definitions.ts` and `export:definitions`, but no scheduler host: most applications do not declare schedules. After adding one, export `purista.schedules.json` during build or deployment and create an isolated host with a shared EventBridge plus an explicit provider. A process-local `DefaultEventBridge` cannot carry events to a separate application process. Do not generate provider credentials, Redis prefixes, broker URLs, or authorization policy from framework defaults.
