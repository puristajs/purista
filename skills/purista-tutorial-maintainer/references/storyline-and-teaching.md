...
- [Page continuity and artifact ledger](#page-continuity-and-artifact-ledger)
...
## Page continuity and artifact ledger

Write one internal artifact ledger before drafting prose. Use one row per
Framework artifact or runtime dependency:

| Artifact | Generated by | Learner edit | Registered by | Runtime binding | Focused proof |
| --- | --- | --- | --- | --- | --- |
| `answerTransactionQuestion` | `npm run add:agent ...` | agent schemas, explicit purpose model alias, prompt, and instructions | `supportHarness.addAgent(...)` and `.mountHarness(...)` | exact `ai.models.answering` binding | standalone Harness and EventBridge tests |

The ledger prevents a common tutorial failure: a page shows a plausible
snippet, but never shows where the symbol is imported, registered, mounted,
injected, called, or tested. Audit every required page against the ledger.

Apply these rules to code fences:

- Say whether the fence creates, replaces, or extends the titled file. Prefer
  a complete file when a beginner could not place a partial snippet safely.
- Include every import needed by the shown code. If an earlier page created a
  symbol, link to that step and keep the same name and path.
- Show registration and runtime composition close to a new definition. A
  command builder without `.addCommandDefinition(...)`, a Harness target
  without `.mountHarness(...)`, or a resource without `getInstance({
  resources })` is incomplete.
- Do not put pseudocode identifiers inside runnable fences. Use prose or an
  explicitly labeled abbreviated excerpt when the complete code is available
  in a previously named file.
...
binding, owning command and its business guard, address-first caller, provider
