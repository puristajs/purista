...
export async function createHttpService(input: {
	...
	// PURISTA 3.2.4's HTTP adapter resets non-JSON responses to text/plain.
	// Restore download headers only for successful CSV responses, never errors.
	http.app.use('/api/v1/accounts/:accountId/statement.csv', async (context, next) => {
		await next()
		if (context.res.status === 200) {
			context.header('content-type', 'text/csv; charset=utf-8')
			context.header('content-disposition', 'attachment; filename="statement.csv"')
		}
	})
