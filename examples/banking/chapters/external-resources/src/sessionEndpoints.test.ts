...
describe('Identity session endpoints', () => {
	...
	test('preserves downstream authorization errors after authentication', async () => {
		const app = await createApplication(initLogger('fatal'))
		try {
			const { sessionToken } = await login(app)
			const response = await app.http.app.request('/api/v1/accounts/account-review/transactions', {
				method: 'POST', headers: { authorization: `Bearer ${sessionToken}`, 'content-type': 'application/json' },
				body: JSON.stringify({ amountCents: 2599, direction: 'debit', counterparty: 'Northwind Books' }),
			})
