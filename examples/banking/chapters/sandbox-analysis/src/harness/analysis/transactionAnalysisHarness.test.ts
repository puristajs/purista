...
import { transactionAnalysisHarness } from './transactionAnalysisHarness.js'

describe('transactionAnalysisHarness', () => {
	it('runs the declared write, bash, and read loop', async () => {
		...
		const runtime = await transactionAnalysisHarness.getInstance({
	...
	it('rejects a write outside the allowed workspace before changing the host fixture', async () => {
