...
export function createGroundedSupportHarness(provider: ModelProvider, knowledge: SupportKnowledge) {
  return defineHarness({ name: 'grounded-support' })
    ...
    .agents(({ agent }) => ({
      answerer: agent({
        model: 'assistant',
        input: z.object({ question: z.string(), evidence: z.array(z.object({ id: z.string(), title: z.string(), excerpt: z.string() })) }),
        output: answer,
        builtinTools: false,
