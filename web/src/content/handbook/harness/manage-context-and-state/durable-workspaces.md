...
export function createArtifactWorkspaceHarness(root: string) {
  ...
  return defineHarness({ name: 'artifact-workspace' })
...
closes its owned storage and workspace adapters in `shutdown()`.

It is a single-host Node/Bun SQLite implementation, not a distributed
production workspace. A production adapter needs durable artifact storage,
transactional coordination with the chosen storage strategy, per-tenant access
control, quota management, encryption/backup policy, and cleanup.
