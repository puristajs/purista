...
Import `claimsMemory` into `src/harness/claimsReview.ts` and pass it as
`memory: claimsMemory` to `definition.getInstance(...)` in the complete Harness definition shown in [SQLite
