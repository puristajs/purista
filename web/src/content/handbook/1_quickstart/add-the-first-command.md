...
describe('service User version 1 - command signUp', () => {
  ...
  test('does not throw', async () => {
    const userService = await userV1Service.getInstance(getEventBridgeMock(sandbox).mock, { logger: getLoggerMock(sandbox).mock })

    const signUp = safeBind(signUpCommandBuilder.getCommandFunction(), userService)
