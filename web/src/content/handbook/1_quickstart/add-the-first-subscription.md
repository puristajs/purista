...
Subscription handler functions must return an outcome object that tells the event bridge how to handle the message. The return value must be one of: `{ status: 'ack' }`, `{ status: 'retry' }`, `{ status: 'deadLetter' }`, `{ status: 'drop' }`, or `{ status: 'stop-consumer' }`.
If a subscription also emits a value, it will be sent as a custom message to the event bridge.
...
- try to use standard prefixes, like `createBankAccount` or `updateUserProfile`
:::
...
Use the arrow keys to select the service (and version), on which the new subscription should be registered.
In our example, choose _Email_ .
...
Now, the files will be created, the event name will be added as a new enum entry and the subscription will be added to the subscription list of the service.

## Structure

A folder for the new subscription is created - in our example it is `src/service/email/v1/subscription/sendWelcomeEmail`.
...
- [API documentation: SubscriptionDefinitionBuilder - Methods](/handbook/api/classes/_purista_core.SubscriptionDefinitionBuilder/#methods)
:::
