...
| [`addPayloadSchema(schema, contentType?, contentEncoding?)`](/handbook/api/classes/_purista_core.CommandDefinitionBuilder/#addpayloadschema) | The validated external invoice input and inferred handler payload. | The optional representation values retain earlier values or resolve to JSON and UTF-8. Invalid input is rejected before guards and cannot reach the repository. |
...
| Reject a command regardless of how it arrives | `setBeforeGuardHooks(...)` | The check runs after input validation and before the handler |
