...
| `protectHandler` | Unset | Middleware used only by endpoints with HTTP security enabled (the builder default). Startup fails when a protected endpoint exists without this middleware. | Authenticate the request, verify and decode its token, and set normalized principal/tenant values. Prefer `setProtectMiddleware(...)` for typed variables. Do not put business authorization here. |
