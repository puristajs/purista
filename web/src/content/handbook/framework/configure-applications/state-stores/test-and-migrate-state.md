...
Next: return to [persist application state](/handbook/framework/configure-applications/state-stores/) to choose an adapter, or use [keys, namespaces, isolation, and consistency](/handbook/framework/configure-applications/state-stores/keys-namespaces-isolation-and-consistency/) when designing the next record.
