...
Queue bridges provide the persistence, leasing, and dead-letter mechanics for pull-based workloads. They are independent from [event bridges](../eventbridges/index.md), so you can mix RabbitMQ/MQTT/NATS for push traffic with any queue backend you prefer. See the [Event Bridge page](../eventbridges/index.md#queue-bridge-support) for a pairing matrix that maps each event bridge to its recommended queue bridge companion.
