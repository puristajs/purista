...
## Declaring an HTTP endpoint on a command

Before wiring the HTTP server, declare which commands to expose using `.exposeAsHttpEndpoint()` on the command builder:

```typescript
import { userServiceV1ServiceBuilder } from '../userServiceV1ServiceBuilder.js'

export const createOrderCommandBuilder = userServiceV1ServiceBuilder
  .getCommandBuilder('createOrder', 'create a new order')
  .addPayloadSchema(createOrderInputSchema)
  .addParameterSchema(createOrderParameterSchema)
  .addOutputSchema(createOrderOutputSchema)
  .exposeAsHttpEndpoint('POST', 'orders')         // HTTP method + path segment
  .addOpenApiTags('Orders')                        // OpenAPI tag grouping
  // By default endpoints are protected; call .makeEndpointPublic() to skip auth:
  // .makeEndpointPublic()
...
The path is relative to the service route prefix. For `UserService v1` with path `orders`, the full URL becomes `/api/v1/user/orders`.
