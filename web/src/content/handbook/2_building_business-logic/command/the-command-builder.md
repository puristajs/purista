...
For adding a command to a service, use the command builder. It is recommended to use the `getCommandBuilder` method of your service builder instance.
...
The context object is the first argument passed to the command function. It provides access to logging, resources, stores, cross-service calls, and message metadata.
...
const myCommandBuilder = myServiceBuilder
  ...
  .setAfterGuardHooks({ // [!code ++]
    first: async function (_context, result, originalPayload, originalParameter) { // [!code ++]
      // the guard implementation // [!code ++]
    }, // [!code ++]
  }) // [!code ++]
  .setCommandFunction(async function (context, payload, parameter) {
