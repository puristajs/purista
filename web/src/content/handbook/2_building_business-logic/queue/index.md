...
The difference between a subscription and a queue comes down to who controls delivery timing. With subscriptions the broker pushes messages to your handler as they arrive, which is the right model for reactive side effects with bounded retry windows. With queues, your worker pulls the next job when it is ready, which is the right model when the work itself is expensive, long-running, or needs operator-visible backlog management. A checkout command might trigger a `paymentReceived` event (subscription territory) and separately enqueue a PDF receipt generation job (queue territory) — both happen after the same business event but with very different delivery and scaling characteristics.

Queue bridges are separate from event bridges, which is an intentional design choice. It means you can evolve your async job infrastructure independently of your event-driven messaging. In development or tests you can rely on the in-memory `DefaultQueueBridge` without any external dependencies, then swap in a durable bridge for production by changing a single constructor call.
...
- [Enterprise long-running queues](../../6_integrations/enterprise_interoperability/long-running-queues.md) – event-to-queue handoff, long-running profiles, and result events.
...
- [Enterprise interoperability](../../6_integrations/enterprise_interoperability/)
