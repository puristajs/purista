...
Complete [Tools & Skills](/handbook/harness/guide/tools-and-skills/) first.
...
| Stdio | A reviewed local MCP server must run beside the agent | Application-owned `dataDirectory` outside the plugin root and a sandbox that supports spawning **and immutable package mounts** | `inMemorySandbox()` or the local host-directory sandbox can enforce an immutable mount |
...
See [Sandboxing & MCP](/handbook/harness/guide/sandboxing-and-mcp/) before
...
Continue with [Sandboxing & MCP](/handbook/harness/guide/sandboxing-and-mcp/)
