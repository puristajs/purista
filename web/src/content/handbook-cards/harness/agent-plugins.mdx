...
flowchart LR
  ...
  select --> harness[Direct Harness definitions]
...
const bindings = plugin.bindings({
	skills: { claimsPlaybook: { runtimes: [] } },
	mcpServers: {
		claimsKnowledge: {
			server: 'knowledge',
			tools: {
				searchClaimsKnowledge: {
					remoteName: 'search',
...
const reviewer = defineAgent('reviewer', {
	model: 'assistant',
	skills: [bindings.skills.claimsPlaybook],
	tools: [bindings.mcpServers.claimsKnowledge.tools.searchClaimsKnowledge],
	instructions: 'Use the claims playbook and the selected search tool when needed.',
})

const definition = defineHarness({ name: 'claims' }).addAgent(reviewer)
const harness = await definition.getInstance({
	models: { assistant: { provider, model: 'provider-model' } },
	mcp: bindings.mcp,
})
```
...
- Use stateless Streamable HTTP or a reviewed stdio server for MCP transport.
