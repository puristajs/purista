...
## Bound temporary state

State is permanent unless you choose a lifetime. Put the shortest useful
retention rule at the scope that owns it:

```typescript [one-value.ts]
// One password-reset token expires after 15 minutes.
await context.states.setState(`password-reset:${tokenId}`, record, {
  retention: { mode: 'expire', ttlMs: 15 * 60_000 },
})
```

```typescript [one-service.ts]
// Every checkout draft written by this service expires after one day.
...
An explicit write policy wins over the service default; use
`{ retention: { mode: 'forever' } }` for an intentional exception. A successful
write refreshes expiry, while reads never do. A requested expiry fails rather
than becoming permanent when the adapter cannot atomically write the value and
its deadline.

| Adapter | Per-write expiry |
|---|---|
| Redis | Yes |
| Dapr | Only with `supportsTtl: true` and a TTL-capable component |
| NATS KV | No; bucket max age is not a per-key sliding TTL |
| DefaultStateStore | Yes, for development and tests only |

See the [StateStore retention guide](/handbook/2_building-business-logic/stores/state-stores/#retention)
for store-instance defaults, exact precedence, and custom adapter requirements.
...
import { StateStoreBaseClass, StateStoreConfig } from '@purista/core'
...
export class MyStateStore extends StateStoreBaseClass<MyStateConfig> {
  ...
  constructor(config: StateStoreConfig<MyStateConfig>) {
