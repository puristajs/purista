...
| **Harness workflow** | `setHarnessWorkflow(definition, { agents })` | Wrap one reusable `@purista/harness` workflow definition and optional harness-local agents. |
...
Agents default to an ephemeral session. Use `setSessionPolicy(...)` when a payload field should identify a continuing AI conversation:

```typescript
.setSessionPolicy({ mode: 'conversation', payloadPath: ['conversationId'] })
```

`conversationId` is required. PURISTA automatically adds trusted message
`tenantId` and `principalId` as optional namespace dimensions when present, so
applications do not configure conversation tenancy themselves.
