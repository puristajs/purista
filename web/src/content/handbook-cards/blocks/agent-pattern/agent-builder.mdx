...
| `setHarnessWorkflow(...)` | You already have a reusable `@purista/harness` workflow definition; pass `{ agents }` when it calls harness-local agents. |
...
supportV1ServiceBuilder.addAgentDefinition(supportAgentBuilder.getDefinition())
```
