...
In this shape, PURISTA owns the outer queue, command, stream, HTTP exposure, and runtime wiring. The Harness workflow owns the inner reasoning loop, and the supplied Harness-local agents share the same session, sandbox, memory, telemetry, Harness storage, durable workspace, and model bindings.
