...
Use `supportV1ServiceBuilder.harnessTarget(...)` to bind the direct workflow
contract to its service address. The client preserves the Harness result
envelope until the command checks for completion. A workflow run can also
return an interrupt, so the command narrows `result.outcome.status` before it
reads `output`. This HTTP wrapper reports an interrupted run as `409 Conflict`
with the run and interrupt descriptors; a continuation endpoint can use those
descriptors to resume the same durable root.
