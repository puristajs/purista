...
Test the portable Harness fan-out with separate strict providers. This proves
both agents run, their typed results merge deterministically, and neither alias
receives an unexpected call.

```ts title="src/harness/support/supportCaseAnalysisHarness.test.ts" write
...
```
Test the command with its mocked workflow address and case policy. The test
asserts trusted identity, the derived session id, and the complete output.
...
```
Use a real local EventBridge for the mounted integration. The denied test calls
the published workflow address directly and proves the mount guard rejects it
before either provider receives work.
