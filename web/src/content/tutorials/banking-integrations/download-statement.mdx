---
title: "Configure the HTTP download response"
description: "Return successful exports as downloadable CSV while preserving the status and content type of errors."
order: 470
---

The command now declares CSV output. Add a small HTTP response wrapper for
this one download route by replacing `src/httpApp.ts`:
...
The new middleware is the `http.app.use` block before the login routes.
It waits for the command response and adds download headers only when the
status is 200. A 401, 403, or 413 keeps its error response and must never look
like a successful CSV download.

The pinned PURISTA 3.2.4 Hono adapter uses Hono's text helper for non-JSON
responses, which replaces the declared content type with `text/plain`.
This wrapper restores `text/csv; charset=utf-8` for the successful export.
The framework source has a correction; retain this compatibility code while
using the package version pinned by this tutorial.

`Content-Disposition` gives browsers a fixed download filename. It does not
use arbitrary client text as a filename. The account scope is already enforced
by the command; a filename is not an access-control mechanism.

The existing session, origin, and protected-command middleware remain intact.
Next, [replace the export test and verify the response bytes](/tutorials/banking-integrations/test-csv-export/).
