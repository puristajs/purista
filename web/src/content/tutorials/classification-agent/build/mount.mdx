...
```
`defineHarnessPolicy` checks the flat `agents` and `workflows` maps against this
Harness definition. The before guard runs before the model provider is called.
Later agents, workflows, tools, skills, and MCP servers can be added to this
same service-owned Harness definition.
