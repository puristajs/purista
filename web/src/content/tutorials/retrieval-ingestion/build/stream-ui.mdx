---
title: Expose an AI SDK UI stream
description: Invoke the answer workflow through a PURISTA stream and emit standard AI SDK UI Message Stream v1 events.
order: 211
kind: lesson
status: draft
---

Extract the latest user text from standard UI messages and reject an empty
conversation with a handled client error.
...
Replace the generated stream. It authorizes the collection, invokes the mounted
workflow through its declared address, and converts native Harness events to AI
SDK UI Message Stream v1.
...
Register both commands, the stream, and the Harness mount on Knowledge.
...
The browser needs no PURISTA-specific client. Any compatible AI SDK UI client
can consume the endpoint's SSE stream, including text deltas and tool status.

```bash title="Type-check the streaming boundary"
npm run build
```

Continue with [Store opaque sessions](../identity-sessions/).
