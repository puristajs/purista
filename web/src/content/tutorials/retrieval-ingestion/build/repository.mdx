---
title: Implement the pgvector repository
description: Make revision replacement atomic and keep similarity search inside the trusted tenant and collection.
order: 206
kind: lesson
status: published
---

Create the PostgreSQL repository resource. It stores chunks and queries the vector index while keeping the database details outside the service commands.
...
```
Check your work: Start the database with `docker compose up -d --wait`, then run `npm run test:postgres`.

Continue with [Build retrieval workflows and commands](../retrieve/).
