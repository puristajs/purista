...
Install the UI packages from the project root. These are normal published npm
dependencies, just like the Framework and Harness packages.

```bash title="Install the React UI packages" replay="project"
npm install react react-dom ai @ai-sdk/react
npm install --save-dev vite @vitejs/plugin-react tailwindcss @tailwindcss/vite typescript @types/react @types/react-dom
```

Create `ui/components.json` so the component generator knows where the React
source and Tailwind stylesheet live.
...
Run the AI Elements generator inside `ui`. It writes the selected components
...
Configure Vite to build the UI into `public`. Hono will serve that directory
in the next step.
...
message parts while text deltas arrive. The endpoint path and bearer token are
the only application-specific transport settings.
