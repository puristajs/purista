...
The `searchKnowledge` tool belongs to `knowledgeV1ServiceBuilder`. Its
`canInvokeWorkflow` declaration therefore uses that same builder's
`harnessTarget(...)` helper to bind the workflow contract to the caller's
service address. This gives the tool a typed `context.workflow.Knowledge['1']`
client and keeps invocation authorization attached to the service that owns
the tool.
