---
title: Store Identity sessions in StateStore
description: Create opaque login sessions in the Identity service and resolve them on the server.
order: 212
kind: lesson
status: published
---

Create the small local identity service. Login creates an opaque session, and the resolver turns that session into the authenticated request context.
...
```
Check your work: Run `npm run build:server`. The identity service compiles with its session types.

Continue with [Protect Hono endpoints](../authenticate-http/).
