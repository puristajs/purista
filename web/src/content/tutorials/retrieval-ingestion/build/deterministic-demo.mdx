...
description: Exercise the real Knowledge services, guards, pgvector repository, Hono endpoints, and stream with fake models.
order: 221
kind: lesson
status: draft
---

Create a credential-free demo entry point. It exercises the retrieval flow with deterministic data so a learner can observe the core behavior without an LLM key.
...
```
Check your work: Run `npm run demo`. The terminal prints the deterministic retrieval result.

Continue with [Test ingestion and retrieval](../../testing/test-rag/).
