...
description: Verify the native agent chooses the host tool and returns validated evidence without a live provider.
order: 223
kind: lesson
status: draft
---

Test the generated retrieval agent through a native Harness runtime. Supply the
host tool implementation as a test adapter and script the model's tool call and
structured result.
...
This proves the LLM-facing contract separately from the PURISTA command binding
added by the service mount.

```bash title="Run the retrieval-agent test"
npx vitest run src/harness/knowledge/agent/retrieveEvidence/retrieveEvidenceAgent.test.ts
```

Continue with [Test the streaming answer workflow](../test-workflow/).
