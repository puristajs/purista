...
Create stream and application tests. They verify the standard UI message stream endpoint and the protected HTTP application wiring. The fake stream implements the complete public handle: `runId`, the server-derived `sessionId`, `result`, `terminal`, `cancel`, and the async event iterator. Approval tests resume the descriptor through `target.resume(descriptor).stream(...)` while keeping the trusted session id supplied by the server.
