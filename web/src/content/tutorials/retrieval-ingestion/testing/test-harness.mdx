...
description: Script model tool selection and verify portable tool, text, and completion events without a provider call.
order: 212
kind: lesson
status: draft
---

The native Harness test owns agent and workflow behavior. Queue each model turn
in order: choose the search tool, return structured evidence, then stream the
grounded answer. Supply a host-tool mock because EventBridge routing belongs to
the integration test on the next page.
...
```
The assertions cover the observable contract: the model selected the declared
tool, tool progress was emitted, text arrived incrementally, and the workflow
finished with the aggregated answer. `provider.assertExhausted()` also catches
unexpected model calls.

```bash title="Run the native Harness RAG test" replay="project"
npm run test:server -- src/harness/knowledge/knowledgeHarness.test.ts
```
