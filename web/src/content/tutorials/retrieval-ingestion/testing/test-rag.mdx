---
title: Test ingestion and retrieval commands
description: Use deterministic Harness models and mocked business resources while executing real command guards and handlers.
order: 222
kind: lesson
status: draft
---

Test ingestion with PURISTA's command context, an in-memory repository, and a
fake embedding provider. Verify authorization, chunk storage, model identity,
...
Test search independently with a deterministic query embedding and mocked
repository.
...
npx vitest run src/service/knowledge/v1/command/ingestKnowledge/ingestKnowledgeCommandBuilder.test.ts src/service/knowledge/v1/command/searchKnowledge/searchKnowledgeCommandBuilder.test.ts
```
