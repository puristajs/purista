---
title: Test the aggregate answer command
description: Verify address first agent invocation, durable approval interruption, acceptance, rejection, and cancellation boundaries.
order: 224
kind: lesson
status: published
---

Create the answer command test. It checks that a request is handed to the durable answer workflow with the correct access context.

```ts title="src/service/knowledge/v1/command/runAnswerKnowledgeQuestion/runAnswerKnowledgeQuestionCommandBuilder.test.ts" write
...
```
Check your work: Run `npm run test:rag-retrieval`. The answer command test passes with the retrieval suite.

Continue with [Test the stream and authentication](../test-stream-http/).
