---
title: Test the streaming answer workflow
description: Verify retrieved evidence becomes cited text deltas and native events are forwarded in order.
order: 224
kind: lesson
status: draft
---

Test the native workflow with a fake retrieval result and scripted streaming
model. Collect its events and assert tool status, text deltas, citations, and
terminal output.
...
```bash title="Run the answer-workflow test"
npx vitest run src/harness/knowledge/workflow/answerKnowledgeQuestion/answerKnowledgeQuestionWorkflow.test.ts
```

Continue with [Test streaming and authentication](../test-stream-http/).
