---
title: Test pgvector storage
description: Prove atomic revision replacement, rollback, similarity search, and tenant isolation against PostgreSQL.
order: 226
kind: lesson
status: draft
---

Create the PostgreSQL integration test. It verifies that the repository writes chunks and returns vector-ranked matches against the real local database.
...
```
Check your work: Run `docker compose up -d --wait`, then `npm run test:postgres`. Vitest reports the PostgreSQL suite passing.

Continue with [Test the browser client](../test-ui/).
