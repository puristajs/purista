...
The runner creates one native Harness runtime, uses one stable session for each case, releases every session, and closes the runtime. Its one-at-a-time concurrency makes the strict fixture order visible.
...
export async function runClassificationEvaluation(
	...
	const harness = await supportHarness.getInstance({
		models: { classification: { provider, model: options.classificationModel ?? 'evaluation-model' } },
	})
	try {
