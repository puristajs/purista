...
```
`classification` is the explicit purpose model alias used by both the
evaluation runtime and mounted service. An agent is the bounded model loop. It
owns the prompt and schemas; routing and authorization stay in PURISTA service
files.
