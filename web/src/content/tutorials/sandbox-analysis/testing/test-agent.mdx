---
title: Test the native tool loop
description: Verify allowed tools complete and denied commands fail before execution in the native Harness runtime.
order: 290
kind: lesson
status: draft
---

Test the generated agent module through a native Harness instance. The fake
provider drives the real built-in tools against a temporary local sandbox.
...
The first case proves the complete write, process, read, and structured-output
loop. The second asks for a forbidden command and proves the Harness permission
layer rejects it before sandbox execution.

```bash title="Run the native agent tests"
npx vitest run src/harness/analysis/agent/analyzeTransactions/analyzeTransactionsAgent.test.ts
```
