...
Start with the strict input and output contracts shared by the agent and the
PURISTA command.
...
```
Fill the generated agent module. It opts into only the file and process tools
required by the analysis, restricts writable paths and commands, and returns a
strict result. The sandbox adapter itself remains a runtime dependency.
...
```
Compose the generated module with its required model alias. Provider and model
selection still happen in the application composition root.
