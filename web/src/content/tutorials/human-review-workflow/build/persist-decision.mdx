---
title: Persist review decisions
description: Store tenant-scoped decisions atomically with optimistic revisions and idempotent event delivery.
order: 256
kind: lesson
status: draft
---

Implement the Support review resource with SQLite. This is the durable business
record. It is separate from Harness checkpoint storage and from PURISTA
StateStore.
...
The `decide` method is one database transaction. It loads the tenant-scoped
pending review, compares the expected revision, and writes the decision,
reviewer, event ID, and next revision atomically. An exact redelivery of the
same event returns the existing result. A stale revision or different terminal
decision becomes `409 Conflict`.

This example uses Node's built-in SQLite driver to keep local setup small. A
production adapter can implement the same `SupportReviewStore` contract with
the database and transaction mechanism used by the application.

```bash title="Type-check the SQLite resource"
npm run build
```

Continue with [Protect the approved effect](../protect-effect/).
