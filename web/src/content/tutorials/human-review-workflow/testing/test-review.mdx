---
title: Test the routed review flow
description: Verify waiting, decisions, authorization, tenant isolation, deadlines, and idempotency over the real EventBridge.
order: 263
kind: lesson
status: draft
---

Finish with the real PURISTA route. Start both service instances on a
`DefaultEventBridge`, mount the native Harness workflow, and keep only the
business adapters in memory.
...
The suite covers the boundaries that can fail when the pieces are composed:

- approval resumes the original run and a duplicate delivery executes the
  effect once;
- rejection never executes the effect;
- the reviewer business policy runs before the decision is recorded;
- tenant isolation prevents another tenant from finding the review;
- a missing durable wait becomes a handled conflict;
- a deadline produces `expired` without the approved effect; and
- stale revisions and conflicting terminal decisions are rejected.
...
No model provider is needed because this workflow coordinates application work
and human input without an LLM.
