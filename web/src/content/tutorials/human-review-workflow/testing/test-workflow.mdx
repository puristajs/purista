---
title: Test the approval workflow
description: Start both services and prove the durable interruption and resume path through PURISTA.
order: 261
kind: lesson
status: published
---

The workflow matters as part of the mounted service, so test it through
`DefaultEventBridge`. This exercises the PURISTA command, mounted workflow,
child agent, approval interruption, resume, host tool, and Transaction command
together. A separate workflow unit test would only copy implementation details.

The strict fake model always asks for `freezeReviewedCard` and then finishes.
The in-memory review store keeps business state separate. Harness uses its real
SQLite storage adapter with an in-memory database, so the durable graph runs
against a supported persistent implementation without leaving files behind.
...
Run the first integration case as an early checkpoint. It starts with the
requester's trusted identity, returns `waiting`, resumes from a same-tenant
reviewer command, and proves duplicate delivery causes one effect.

```bash title="Run the workflow approval checkpoint" replay="project"
npx vitest run src/humanReview.integration.test.ts -t "returns waiting, resumes approval"
```

Continue with [Test commands and guards](../test-commands/).
