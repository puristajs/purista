---
title: Test the mounted EventBridge path
description: Prove the wrapper, mounted target policy, shared transcript storage, and denial path together.
order: 229
kind: lesson
status: published
---

Finish with a mounted-service test. The allowed case invokes the wrapper,
exports the transcript through the authorized resource, clears it, and confirms
it is empty. This proves the agent and history commands use one session id and
one storage adapter.

The denied case uses a test-only command with a declared agent call. Because
that command belongs to `directCallerBuilder`, its
`directCallerBuilder.harnessTarget(...)` declaration binds the target contract
to the caller service. The mounted target guard rejects it before the strict
fake model receives a request.
...
```
```bash title="Run the complete local verification" replay="project"
npm run build
npm test
npm run lint
npm run demo
...
All automated checks are credential-free. Before sharing a different storage
adapter between application instances, run the Harness storage contract suite
for that adapter too.
