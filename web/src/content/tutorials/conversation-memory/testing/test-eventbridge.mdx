...
The denied case uses a test-only command with a declared, address-first agent
call. The mounted target guard rejects it before the strict fake model receives
a request.
