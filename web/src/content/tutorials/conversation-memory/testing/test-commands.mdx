...
description: Prove trusted session derivation and action-specific authorization with PURISTA command testing helpers.
order: 228
kind: lesson
status: draft
---

The native tests prove Harness history behavior. Now test the PURISTA command
boundary without starting EventBridge. These assertions verify that agent
invocation and transcript deletion derive their ids from trusted message
identity, and that the policy receives distinct `continue` and `clear` actions.

Create the complete command test file.
...
`createCommandContextMock(...)` reads each builder declaration and creates the
matching typed agent or resource stub. Setting `context.message` separately is
what lets the test prove the session key uses trusted message identity rather
than payload data.

```bash title="Run the conversation command tests" replay="project"
npm test -- src/service/support/v1/command/conversationCommands.test.ts
```

Continue with [Test conversation routing through EventBridge](../test-eventbridge/).
