...
Create the shared request, transcript, and clear-result schemas. The service
configuration is empty in this focused example, but declaring it keeps the
service builder and instance contract explicit.
...
```
Declare the narrow transcript resource that the read and clear commands will
use. Harness still owns the storage implementation and session lifecycle.
...
```
Build one logical conversation id, then derive the opaque storage id only when
an application command needs to inspect stored messages. The mounted runtime
uses the same `createHarnessSessionStorageId(...)` helper internally.
...
EventBridge. Reusing the same logical session id loads earlier complete turns.
The mounted runtime scopes it with trusted tenant and principal identity, so a
different caller cannot join the transcript by supplying the same
`conversationId`.
