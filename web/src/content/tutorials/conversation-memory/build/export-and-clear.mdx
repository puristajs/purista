...
the Harness runtime and the narrow history resource so the resource does not
...
The live composition root creates SQLite and the OpenAI adapter, then passes
both into this function. Tests and the deterministic demo pass in-memory
storage and a strict fake provider through the same interface.
