...
While PURISTA offers support for a wide range of schema libraries via the [Standard Schema](https://standardschema.dev) spec, it is strongly recommended to utilize [Zod](https://zod.dev/) as the primary schema library. This recommendation is due to the extensive ecosystem surrounding Zod, which enables the implementation of advanced features like OpenAPI schema generation.
...
If you have the need to use other schema and validation libraries, ensure they implement the Standard Schema v1 interface.  
Please have a look at the official documentation at [standardschema.dev](https://standardschema.dev) to see the specification details.

In addition to the schema library itself, you might need additional packages as well, to convert the validation schema into [JSON Schema](https://json-schema.org).
