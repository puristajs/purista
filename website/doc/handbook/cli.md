...
```
Use `purista add agent` to scaffold an AI workload manifest powered by `@purista/ai`. The generator now creates an `AgentBuilder` under `src/agents/<name>/v<version>/` plus a starter Vitest spec. Agents run beside services—no helper command is required. After filling in the handler (or swapping the placeholder model resource), call `agentDefinition.getInstance()` inside your bootstrap and invoke it via `invokeAgent` or the optional queue workers shipped with `@purista/ai`.
